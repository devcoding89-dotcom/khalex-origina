...
export default function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Ensure customer number is clean for WhatsApp
  const cleanCustomerNumber = order.whatsapp.replace(/\D/g, '');
  const waLink = `https://wa.me/${cleanCustomerNumber}?text=Hi ${order.customerName}! Regarding your order ${order.id}...`;
          ...
          <div className="lg:col-span-2 space-y-8">
            <Card className="bg-card border-primary/20">
              ...
              <CardContent className="pt-6 space-y-6">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="space-y-1">
                    <div className="text-[8px] text-muted-foreground uppercase font-black">Order ID</div>
                    <div className="font-mono text-xs">{order.id}</div>
                  </div>
                  <div className="space-y-1">
                    <div className="text-[8px] text-muted-foreground uppercase font-black">Date</div>
                  ...
                  <div className="space-y-1">
                    <div className="text-[8px] text-muted-foreground uppercase font-black">Payment</div>
                ...
                <div className="border-t border-primary/5 pt-6">
                  <div className="text-[10px] font-black uppercase tracking-widest mb-4">Items Ordered</div>
              ...
              <CardFooter className="bg-muted/10 border-t border-primary/5 p-6 flex justify-between items-center">
                <div className="text-xl font-black uppercase italic">Total</div>
            ...
            <Card className="bg-card border-primary/10">
              <CardHeader>
                <CardTitle className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                  <Clock className="w-4 h-4 text-primary" /> Update History
                </CardTitle>
              </CardHeader>
          ...
          <div className="space-y-8">
            <Card className="bg-card border-primary/20 shadow-xl overflow-hidden">
              <CardHeader className="bg-primary/5 border-b border-primary/10">
                <CardTitle className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                  <Send className="w-3 h-3 text-primary" /> Order Controls
                </CardTitle>
              </CardHeader>
              <CardContent className="p-6 space-y-6">
                <div className="space-y-2">
                  <Label className="uppercase font-black text-[9px] tracking-widest text-muted-foreground">Note for Customer</Label>
                  <textarea 
                    value={adminNote}
                    onChange={(e) => setAdminNote(e.target.value)}
                    placeholder="Update the customer on their order..."
                ...
                <div className="space-y-4">
                  <div className="text-[8px] text-muted-foreground uppercase font-black">Change Order Status</div>
                ...
                <div className="space-y-2 pt-6 border-t border-primary/5">
                  <div className="text-[8px] text-muted-foreground uppercase font-black">Financial Status</div>
                  <div className="grid grid-cols-2 gap-2">
            ...
            <Card className="bg-card border-primary/10 overflow-hidden">
              ...
              <CardContent className="p-6 space-y-4">
                <div>
                  <div className="text-[8px] text-muted-foreground uppercase font-black">Name</div>
                  <div className="text-sm font-bold uppercase">{order.customerName}</div>
                </div>
                <div>
                  <div className="text-[8px] text-muted-foreground uppercase font-black">WhatsApp</div>
                ...
                {order.codUid && (
                  <div className="p-3 bg-secondary/5 rounded border border-secondary/20 space-y-2">
                    <div className="text-[8px] text-secondary uppercase font-black">Game UID</div>
                    <div className="text-xs font-mono">{order.codUid}</div>
                    <div className="text-[8px] text-secondary uppercase font-black mt-2">In-Game Name</div>
                ...
                <Button asChild variant="outline" className="w-full border-primary/20 h-10 text-[10px] uppercase font-black">
                  <Link href={`/admin/customers`}>
                    View All Customers <ExternalLink className="w-3 h-3 ml-2" />
                  </Link>
                </Button>
              </CardContent>
            </Card>
